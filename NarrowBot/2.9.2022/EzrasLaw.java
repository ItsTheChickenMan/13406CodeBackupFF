...
public abstract class EzrasLaw extends LinearOpMode {
  ...
  public void pivotToAngle(double orientation, double speed) {
    ...
    if (finalOrientation < startingOrientation) {
      ...
      this.frontLeft.setPower(speed);
      this.frontRight.setPower(-speed);
      while (getYRotation() > finalOrientation && opModeIsActive()) {
        double speedFactor = 1.0D - (getYRotation() - startingOrientation) / (finalOrientation - startingOrientation);
        speedFactor += extraSpeed;
        this.backLeft.setPower(speed * speedFactor);
        this.backRight.setPower(-speed * speedFactor);
        this.frontLeft.setPower(speed * speedFactor);
        this.frontRight.setPower(-speed * speedFactor);
        this.telemetry.addData("starting orientation", Double.valueOf(startingOrientation));
        this.telemetry.addData("final orientation", Double.valueOf(finalOrientation));
        this.telemetry.addData("current gyro", Double.valueOf(getYRotation()));
        this.telemetry.addData("speedFactor", Double.valueOf(speedFactor));
        this.telemetry.update();
      } 
    } else {
      this.backLeft.setPower(-speed);
